"""Class for Crystal Ball distribution."""
...
class cb:
    ...
    def getY(self):
        ...
        A = np.power((self.n / abs(self.alpha)), self.n) * np.exp(
            -(abs(self.alpha) ** 2) / 2
        )
        B = (self.n / abs(self.alpha)) - abs(self.alpha)
        C = (
            (self.n / abs(self.alpha))
            * (1 / (self.n - 1))
            * np.exp(-(abs(self.alpha) ** 2) / 2)
        )
        D = np.sqrt(np.pi / 2) * (1 + erf(abs(self.alpha) / np.sqrt(2)))
        N = 1 / (self.width * (C + D))
        geq = N * np.exp(
            -1
            * np.multiply(self.x - self.mean, self.x - self.mean)
            / (2 * (self.width**2))
        )
        leq = N * A * np.power((B - (self.x - self.mean) / self.width), -1 * self.n)
        mask_geq = (self.x - self.mean) / self.width > -1 * self.alpha
        self.y = np.array(
            [geq[i] if mask_geq[i] else leq[i] for i in range(len(mask_geq))]
        )
