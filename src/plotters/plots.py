...
import src.helpers.helper_plots as helper_plots
...
def plot_style_paper(data, mc, plot_title, **options):
    ...
    Args:
        ...
        options: dictionary of options
            ...
            - _kFit: bool, whether to fit the data and mc
    ---------------------------------------
    Returns:
        None
    ---------------------------------------
    ...
    if options["bins"] and options["bins"] != "auto":
        binning = [
            pc.HIST_MIN
            + float(i) * (pc.HIST_MAX - pc.HIST_MIN) / float(options["bins"])
            for i in range(int(options["bins"]) + 1)
        ]
    ...
    if options["_kFit"]:
        ...
        fit_params_data = fit_bw_cb(
            mids, h_data, [1.424, 1.86, avg_data - 91.188, sigma_data]
        )
        ...
        if options["_kPlotFit"]:
            ...
            plot_style_bw_cb_fit(
                h_data,
                fit_params_data["fit_hist"],
                h_bins,
                f"{plot_title}_data_with_fit",
                fit_title="Fit",
                data_title="Data",
                tag=options["tag"],
            )
            plot_style_bw_cb_fit(
                h_mc,
                fit_params_mc["fit_hist"],
                h_bins,
                f"{plot_title}_mc_with_fit",
                fit_title="Fit",
                data_title="MC",
                tag=options["tag"],
            )
    ...
    syst_unc = np.zeros(len(h_bins) - 1)
    if options["syst"]:
        syst_unc = helper_plots.get_systematic_uncertainty(
            h_bins,
            data,
            data_up,
            data_down,
    ...
    if options["_kFit"] and fit_params_data and fit_params_mc:
        labels[0] = (
            f"{labels[0]}: $\mu$ = {fit_params_data['mu']:.3f} GeV, $\sigma$ = {fit_params_data['sigma']:.3f} GeV"
        )
        labels[1] = (
            f"{labels[1]}: $\mu$ = {fit_params_mc['mu']:.3f} GeV, $\sigma$ = {fit_params_mc['sigma']:.3f} GeV"
        )
    ...
    for text in style.annotations.keys():
        if text == "plot_title":
            ...
            if plot_title in style.annotations[text]["annot"].keys():
                print(
                    "plot_title found in annotations",
                ...
                print(
                    f"WARNING: {plot_title} not found in annotations, no plot title will be added"
                )
        else:
            lumi = None
            if text == "lumi":
    ...
    # save fig
    print(f"{plot_dir}{style.style}{options['tag']}_{plot_title}.png")
...
def plot_style_bw_cb_fit(hist, fit, bins, plot_title, **options):
    ...
    axs[1].set_ylabel(
        style.labels["ratio"], horizontalalignment="right", y=1.0, fontsize=14
    )
    axs[1].set_xlabel(
        style.labels["x_axis"]["label"],
        horizontalalignment=style.labels["x_axis"]["ha"],
