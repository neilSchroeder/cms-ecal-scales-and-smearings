...
def congruentCategories(last, this, nameLast, nameThis):
    ...
    if round(last[2], 4) == round(this[2], 4) and round(last[3], 4) == round(
        this[3], 4
    ):
        eta = True
    elif round(last[2], 4) <= round(this[2], 4) and round(last[3], 4) >= round(
        this[3], 4
    ):
        # this is the case where the last category is a superset of this category
        eta = True
    ...
    if round(last[4], 4) == round(this[4], 4) and round(last[5], 4) == round(
        this[5], 4
    ):
        r9 = True
    elif round(last[4], 4) <= round(this[4], 4) and round(last[5], 4) >= round(
        this[5], 4
    ):
        # this is the case where the last category is a superset of this category
        r9 = True
    ...
    if round(last[6], 4) == round(this[6], 4) and round(last[7], 4) == round(
        this[7], 4
    ):
        et = True
    elif round(last[6], 4) <= round(this[6], 4) and round(last[7], 4) >= round(
        this[7], 4
    ):
        # this is the case where the last category is a superset of this category
        et = True
...
def combine(thisStep, lastStep, outFile):
    ...
    print(
        "[INFO][python/write_files][combine] producing combined scales file from {} and {}".format(
            thisStep, lastStep
        )
    )
    print(
        "[INFO][python/write_files][combine] this output will be written to {}".format(
            outFile
        )
    )
    ...
    for rowLast in dfLastStep.itertuples(index=False):
        for rowThis in dfThisStep.itertuples(index=False):
            ...
            if int(kCongruent) == -999:
                # now you've done it
                print(
                    "[FATAL ERROR][python/write_files][combine] Since this isn't working, let's just stop"
                )
                return
...
def write_smearings(smears, cats, out):
    ...
    smear_mask = cats.iloc[:, cc.i_type] == "smear"

    for index, row in cats[smear_mask].iterrows():
        if row.iloc[cc.i_type] != "scale":
            if row.iloc[cc.i_r9_min] == -1:
                row.iloc[cc.i_r9_min] = 0
                row.iloc[cc.i_r9_max] = 10
            if row.iloc[cc.i_et_min] != -1:
                dictForDf["#category"].append(
                    f"absEta_{row.iloc[cc.i_eta_min]}_{row.iloc[cc.i_eta_max]}-R9_{round(row.iloc[cc.i_r9_min],4)}_{row.iloc[cc.i_r9_max]}-Et_{row.iloc[cc.i_et_min]}_{row.iloc[cc.i_et_max]}"
                )
            else:
                dictForDf["#category"].append(
    ...
    for index, row in cats.iterrows():
        if row.iloc[0] != "smear":
            for i in range(len(scales)):
                dictForDf["runMin"].append(row.iloc[0])
                dictForDf["runMax"].append(row.iloc[1])
