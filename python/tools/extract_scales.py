...
def congruent(cat, origin, target):
    ...
    if origin[i_eta_min] <= cat[i_eta_min-1] and origin[i_eta_max] >= cat[i_eta_max-1]:
        ret_origin_eta = True
        if origin[i_r9_min] <= cat[i_r9_min-1] and origin[i_r9_max] >= cat[i_r9_max-1]:
            ret_origin_r9 = True
        else:
            return False
    else:
    ...
    if round(target[i_eta_min],4) == cat[i_eta_min-1] and round(target[i_eta_max],4) == cat[i_eta_max-1]:
        ret_target_eta = True
        if target[i_r9_min] == cat[i_r9_min-1] and target[i_r9_max] == cat[i_r9_max-1]:
            ret_target_r9 = True
        else:
            return False
    else:
