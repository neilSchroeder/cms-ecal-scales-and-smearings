...
def minimize(data, mc, cats_df, args):
    ...
    if start_style != "scan":
        ...
        if start_style == "specify":
            scan_file_df = pd.read_csv(start_file, sep="\t", header=None)
            # column 9 is the scales and smearings, grab all of them
            guess = scan_file_df.loc[:, 9].values
            # check length of guess against the number of scales and smearings
            if len(guess) == __num_scales__:  # only scales were specified
                guess = np.append(guess, [0.005 for i in range(__num_smears__)])
            elif (
                len(guess) == __num_scales__ + __num_smears__
            ):  # scales and smearings were specified
                pass
            else:
                print(
                    "[ERROR][python/utilities/minimizer][minimize] The number of scales and smearings in the file does not match the number of scales and smearings in the category definitions."
                )
                return []
