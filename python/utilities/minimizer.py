...
def minimize(data, mc, cats_df, config: MinimizationConfig):
    """
    ...
    Args:
        ...
        config: MinimizationConfig dataclass containing all minimization
            options (hist range, scan parameters, flags, etc.)
    -------------------------------------------------------------
    Returns:
    ...
    ignore_cats = config.ignore  # categories to ignore
    hist_min = round(config.hist_min, 2)  # bottom edge of histogram
    hist_max = round(config.hist_max, 2)  # top edge of histogram
    bin_size = round(config.bin_size, 2)  # bin size
    start_style = config.start_style  # seed method for scales/smearings
    scan_min = config.scan_min  # min value in scan
    scan_max = config.scan_max  # max value in scan
    scan_step = config.scan_step  # step size of scan
    min_step = config.min_step_size  # step size of minimizer
    _kClosure = config._kClosure  # closure flag
    scales = config.scales  # scales file
    _kPlot = config._kPlot  # plot flag
    plot_dir = config.plot_dir  # directory to put plots
    _kTestMethodAccuracy = config._kTestMethodAccuracy  # test method flag
    _kScanNLL = config._kScanNLL  # scan nll flag
    ...
    if dc.MINIMIZATION_STRATEGY == "AdamW":
        ...
        optimum = minz(
            ...
            np.array(guess),
            args=(empty_guess, __ZCATS__, __num_scales__, __num_smears__),
            method=dc.MINIMIZATION_STRATEGY,
            bounds=bounds,
            options=min_step_dict,
        )
