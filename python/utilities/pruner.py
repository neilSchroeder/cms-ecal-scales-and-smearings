...
def prune(files, out, out_dir):
    ...
    print(
        f"[WARNING] if you get a KILLED message, try switching to a machine with more available memory"
    )
    ...
    for line in files:
        ...
        with uproot.open(line_list[2]) as f:
            df = f[line_list[1]].arrays(keep_cols, library="pd")
            ...
            mask_lead = np.logical_or(
                df[dc.ETA_LEAD].values < dc.MAX_EB, dc.MIN_EE < df[dc.ETA_LEAD].values
            )
            mask_lead = np.logical_and(mask_lead, df[dc.ETA_LEAD].values <= dc.MAX_EE)
            mask_sub = np.logical_or(
                df[dc.ETA_SUB].values < dc.MAX_EB, dc.MIN_EE < df[dc.ETA_SUB].values
            )
            ...
            energy_mask = np.logical_and(
                df[dc.E_LEAD].values > dc.MIN_E, df[dc.E_LEAD].values < dc.MAX_E
            )
            energy_mask = np.logical_and(
                energy_mask,
                np.logical_and(
                    df[dc.E_SUB].values > dc.MIN_E, df[dc.E_SUB].values < dc.MAX_E
                ),
            )
            df = df[energy_mask]

            # drop events with invmass less than 60 or greater than 120
            invmass_mask = np.logical_and(
                dc.invmass_min < df[dc.INVMASS].values,
                df[dc.INVMASS].values < dc.invmass_max,
            )
            df = df[invmass_mask]
            ...
            df.to_csv(
                f"{out_dir}{out}_{data_mc}_{index}.csv",
                sep="\t",
                header=True,
                index=False,
            )
