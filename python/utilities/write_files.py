...
def write_runs(runs, out):
    ...
    --------------------------------
    """
    if os.path.dirname(out) and not os.path.exists(os.path.dirname(out)):
        os.makedirs(
            os.path.dirname(out)
        )  # catch if the datFiles/ directory doesn't exist
...
def write_time_stability(scales, runs, outFile):
    ...
    if outFile.find("scales") == -1:
        outFile = outFile.replace(".dat", "_scales.dat")
    print(
...
def write_systematics(systematics, output_tag):
    ...
    for eta_key in systematics.keys():
        for r9_key in systematics[eta_key].keys():
