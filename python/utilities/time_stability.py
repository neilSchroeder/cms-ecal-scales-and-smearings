...
def derive(data, runs, output, _kWriteData=True):
    ...
    for i, pair in run_bins.iterrows():
        ...
        mask_mass = np.logical_and(
            dc.MIN_INVMASS <= data[dc.INVMASS].values,
            data[dc.INVMASS].values <= dc.MAX_INVMASS,
        )
        ...
        for j, eta_bin in enumerate(ret):
            bin_mask = np.logical_and(
                mask,
                np.logical_and(
                    eta_min[j] <= data[dc.ETA_LEAD].values,
                    data[dc.ETA_LEAD].values < eta_max[j],
                ),
            )
            bin_mask = np.logical_and(
                bin_mask,
                np.logical_and(
                    eta_min[j] <= data[dc.ETA_SUB].values,
                    data[dc.ETA_SUB].values < eta_max[j],
                ),
            )
            bin_values = data[bin_mask][dc.INVMASS].values
            n_events = len(bin_values)
            if n_events < 2:
                print(
                    f"[WARNING][time_stability] skipping eta bin {eta_min[j]}-{eta_max[j]}, run {pair[0]}-{pair[1]}: only {n_events} events"
                )
                eta_bin.append(1.0)
                if _kWriteData:
                    ...
                    dictForDf["median"].append(np.nan)
                    dictForDf["mean"].append(np.nan)
                    dictForDf["sigma"].append(np.nan)
                    dictForDf["events"].append(n_events)
                    dictForDf["scale"].append(1.0)
                    dictForDf["median_corr"].append(np.nan)
                    dictForDf["mean_corr"].append(np.nan)
                    dictForDf["sigma_corr"].append(np.nan)
                continue
            eta_bin.append(dc.TARGET_MASS / stat.median(bin_values))
            if _kWriteData:
                dictForDf["run_min"].append(pair[0])
                dictForDf["run_max"].append(pair[1])
                dictForDf["eta_min"].append(eta_min[j])
                dictForDf["eta_max"].append(eta_max[j])
                dictForDf["median"].append(stat.median(bin_values))
                dictForDf["mean"].append(stat.mean(bin_values))
                dictForDf["sigma"].append(
                    stat.stdev(np.array(bin_values, dtype=np.float64))
                )
                dictForDf["events"].append(n_events)
                dictForDf["scale"].append(eta_bin[-1])
                invmass = bin_values * eta_bin[-1]
                invmass = invmass[
                    np.logical_and(dc.MIN_INVMASS <= invmass, invmass <= dc.MAX_INVMASS)
                ]
                if len(invmass) < 2:
                    dictForDf["median_corr"].append(np.nan)
                    dictForDf["mean_corr"].append(np.nan)
                    ...
                    dictForDf["sigma_corr"].append(
                        stat.stdev(np.array(invmass, dtype=np.float64))
                    )

    if _kWriteData:
        dfOut = pd.DataFrame(dictForDf)
        dfOut.to_csv(
            f"{ss_config.DEFAULT_WRITE_FILES_PATH}time_stability_outputData_{output}.dat",
            sep="\t",
            header=True,
            index=False,
        )
