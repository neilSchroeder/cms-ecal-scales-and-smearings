...
from python.utilities.masks import build_dielectron_mask
...
def get_dataframe(
    ...
    if ".root" in files[0]:
        ...
        df = pd.concat(
            [
                uproot.open(f)[dc.TREE_NAME].arrays(dc.KEEP_COLS, library="pd")
                for f in files
            ]
        )
    ...
    if apply_cuts == "standard":
        ...
        df = custom_cuts(df, eta_cuts, inv_mass_cuts, et_cuts, r9_cut, working_point)
...
def extract_cats(
    ...
    for index1 in range(options["num_scales"]):
        for index2 in range(index1 + 1):
            ...
            # select data events matching this dielectron category
            data_mask = build_dielectron_mask(data, cat1, cat2)
            df = data[data_mask]
            mass_list_data = np.array(df[dc.INVMASS])

            # select MC events matching this dielectron category
            mc_mask = build_dielectron_mask(mc, cat1, cat2)
            df = mc[mc_mask]
...
def get_smearing_index(cats: pd.DataFrame, cat_index: int) -> int:
    ...
    matching = cats.loc[truth].index
    if len(matching) == 0:
        raise ValueError(
            f"No smearing category found for scale category {cat_index} "
            f"(eta=[{eta_min},{eta_max}], r9=[{r9_min},{r9_max}], et=[{et_min},{et_max}])"
        )
    return matching[0]
