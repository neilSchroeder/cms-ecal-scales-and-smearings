    ...
    for i, hist in enumerate(hists):
        for eta_key in hist:
            for r9_key in hist[eta_key]:
                ...
                if (
                    sum(hist[eta_key][r9_key][0]) == 0
                    or sum(hist[eta_key][r9_key][1]) == 0
                ):
                ...
                sigma_data = np.sqrt(
                    np.average(
                        (mids - mean_data) ** 2, weights=hist[eta_key][r9_key][0]
                    )
                )
                sigma_mc = np.sqrt(
                    np.average((mids - mean_mc) ** 2, weights=hist[eta_key][r9_key][1])
                )
                data_fit = fit_bw_cb(
                    mids,
                    hist[eta_key][r9_key][0],
                    [1.424, 1.86, mean_data - dc.TARGET_MASS, sigma_data],
                )
                mc_fit = fit_bw_cb(
                    mids,
                    hist[eta_key][r9_key][1],
        ...
        eta_key: {
            r9_key: {
                syst_key: abs(
                    1
                    - (
                        systematics_categories[syst_key][0][eta_key][r9_key]
                        / systematics_categories[syst_key][1][eta_key][r9_key]
                    )
                    / nominal_hists[eta_key][r9_key]
                )
