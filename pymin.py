...
def main():
    ...
    Args:
        ...
        --start-style: Style of starting values for minimization, options are 'scan', 'random', 'specify'. If using specify, used the --only-step argument to provide the starting locations.
    ...
    parser.add_argument(
        "--pruned-file-dest",
        default=ss_config.DEFAULT_DATA_PATH,
        dest="pruned_file_dest",
    ...
    parser.add_argument(
        "-s", "--scales", default=None, help="path to scales file to apply to data"
    )
    parser.add_argument(
        "--smearings", default=None, help="path to smearings file to apply to MC"
    )
    parser.add_argument(
        "-w",
        "--weights",
    ...
    parser.add_argument(
        "-o", "--output", default=None, help="output tag to add to file names"
    )
    parser.add_argument(
        "--closure",
        default=False,
    ...
    parser.add_argument("--queue", default="tomorrow", help="flavour of submitted job")
    parser.add_argument(
        "--from-condor",
        default=False,
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="_kCombine",
        help="[ADVANCED] combines two specified files, using the --only-step and --step options",
    )
    parser.add_argument(
        "--only-step",
        default="",
        type=str,
        dest="only_step",
        help="[ADVANCED] only step file, to be used with the --combine-files and --step options or --start-style specify",
    )
    ...
    parser.add_argument(
        "--plot-dir", default="./", dest="plot_dir", help="directory to write the plots"
    )
    parser.add_argument(
        "--lumi-label",
        default=None,
    ...
    if args._kPrune:
        if args.pruned_file_dest is None:
            print(
                "[ERROR] you have requested a pruned file, but have not specified a destination"
            )
            print("[ERROR] please resubmit with a --pruned_file_dest argument")
            return
        if args.output is None:
            print(
                "[ERROR] you have requested a pruned file, but have not specified an output name"
            )
    ...
    if args._kRunDivide:
        if args.output is None:
            print(
                "[ERROR] you have requested a run divide, but have not specified an output name"
            )
            print("[ERROR] please resubmit with a --output argument")
            return
        try:
            ...
            print(
                "[ERROR] you have requested a run divide, but have not provided a valid minimum number of events"
            )
    ...
    if args._kTimeStability:
        if args.output is None:
            print(
                "[ERROR] you have requested a time stability, but have not specified an output name"
            )
            print("[ERROR] please resubmit with a --output argument")
            return
        if args.catsFile is None:
            print(
                "[ERROR] you have requested a time stability, but have not specified a category"
            )
    ...
    if not args._kNoReweight:
        ...
        if args.weights is None:
            if args.output is None:
                print(
                    "[ERROR] you have requested a weight derivation, but have not specified an output name"
                )
                print("[ERROR] please resubmit with a --output argument")
                return
    ...
    if args._kClosure or args.smearings is not None:
        if not args._kTestMethodAccuracy:
            if args.smearings is None:
                print(
                    "[ERROR] you have requested a closure test, but have not provided a smearings file"
                )
